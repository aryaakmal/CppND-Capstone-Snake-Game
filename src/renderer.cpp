...
void Renderer::Render(Snake const snake, SDL_Point const &food, SDL_Point const &poison, Bomb const bomb, Missile const missile) {
  ...
  // Render poison
  SDL_SetRenderDrawColor(sdl_renderer, 0xFF, 0x55, 0x00, 0xFF);
...
void Renderer::UpdateWindowTitle(int score, int fps, std::string user) {
  std::string title{"Player: " + user + "  -- " +"Snake Score: " + std::to_string(score) + " FPS: " + std::to_string(fps)};
